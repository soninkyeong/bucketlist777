...
public class ContentsController {
	...
	@RequestMapping("/contentscom.bk")
	public String complete(int id) throws Exception{
	...
	public ModelAndView update(int contents_id) throws Exception{
		ModelAndView mv = new ModelAndView("main");
		
		mv.addObject("contentsupdate", biz.get(contents_id)); //biz에 있는 get함수 갔다가 와! contentsupdate는 내가 마음대로 정한이름
	...
	@RequestMapping("/contentsdelete.bk")
	   public String remove(ContentsCommand uc){
	      try {
	    	  System.out.println(uc.getContents_id());
	         System.out.println("전체 : " + uc);
	         System.out.println("아이디 : " + uc.getUser_id()+"컨텐츠 : " + uc.getContents_id());
	         biz.remove(uc);
	      } catch (Exception e) {
	         e.printStackTrace();
	      }
